...
function initials() {//公众号排序
    var SortList=$(".aui-list-item.aui-list-item-middle");
    var SortBox=$("#big");
    SortList.sort(asc_sort).appendTo('#big');//每个用户DIV栏按首字母排序
	//console.log(SortList.length);
	//console.log(JSON.stringify(SortList))

    function asc_sort(a, b) {
		return $(a).find('.aui-list-item-title').text().localeCompare($(b).find('.aui-list-item-title').text(),"zh")
    }
    var initials = [];
    var num=0;
    SortList.each(function(i) {
        var initial = makePy($(this).find('.aui-list-item-title').text().charAt(0))[0].toUpperCase();
	...
	initials.sort((a,b)=>{return a.localeCompare(b)});//给字母标签排序
    $.each(initials, function(index, value) {//添加首字母标签
        SortBox.append('<div class="sort_letter" style="background:rgba(245,245,245,1);" id="'+ value +'">' + value + '</div>');
    });
    if(num!=0){SortBox.append('<div class="sort_letter" id="default">#</div>');}
	for (var i =SortList.length-1;i>-1;i--) {//插入到对应的首字母后面
		let letter=makePy(SortList.eq(i).find('.aui-list-item-title').text().charAt(0))[0].toUpperCase();
		switch(letter){
			case "A":
